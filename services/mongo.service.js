const { UserRVMS } = require("../models/user.model");
const { VehicleRVMS } = require("../models/vehicle.model");
    ...
    return new Promise(async (resolve) => {
      try {
        if (!opts.attributes || opts.attributes.length === 0) {
          error(
            `::DATABASE ERROR :: Attributes is required in (find) method. ${opts.model}`
          );
        }
      ...
      try {
        let query =
          opts.query && Object.entries(opts.query).length >= 1
            ? opts.query
            : {};
        return resolve(await models[opts.model].deleteOne(query));
      } catch (e) {
        error(`::DATABASE ERROR :: catch error in (deleteOne) method. ${e}`);
        return resolve(false);
      }
    });
  },
